...
static void _destroy_tests(struct list_head *suites)
{
	struct test_suite *ts, *tmp;

	list_for_each_entry_safe (ts, tmp, suites, list)
...
int main(int argc, char **argv)
{
	...
	INIT_LIST_HEAD(&suites);
	register_all_tests(&suites);

	// count all tests
	nr_tests = 0;
	list_for_each_entry (ts, &suites, list)
		list_for_each_entry (t, &ts->tests, list)
			nr_tests++;
